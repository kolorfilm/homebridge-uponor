...
export const createUponorThermostatAccessory = (
  ...
  const service: Service = accessory.getService(platform.Service.Thermostat)
  ...
  service.getCharacteristic(platform.Characteristic.CurrentRelativeHumidity)
    .setProps({
      minValue: 0,
      maxValue: 100,
    });
  ...
  service.getCharacteristic(platform.Characteristic.Name)
    .onGet(async (): Promise<CharacteristicValue> => {
